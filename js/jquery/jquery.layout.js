/*
 * jquery.layout 1.3.0 - Release Candidate 29
 *
 * Copyright (c) 2010 
 ...
 *
 * $Date: 2010-06-12 08:00:00 (Sat, 12 June 2010) $
 * $Rev: 3029 $
 * 
 ...
 */
	...
	var sizeContent = function (panes, remeasure) {
		...
		$.each(panes.split(","), function (idx, pane) {
			...
			if (!_c.isLayoutBusy || m.above == undefined || remeasure || o.resizeContentWhileDragging) {
				var
					ignore	= options[pane].contentIgnoreSelector
				,	$Ftrs	= ignore ? $C.nextAll(':visible').not(ignore) : $C.nextAll(':visible')
				,	numFtrs	= $Ftrs.length
				;
				m.above = $C[0].offsetTop;
				m.below = 0; // correct if no content footer
				if (numFtrs) {
					var E = $Ftrs[numFtrs-1]; // Last Footer elem
					// Last-Footer.bottom (top+height) - First-Footer.top
					m.below = (E.offsetTop + E.offsetHeight) - $Ftrs[0].offsetTop; 
				}
			}
			var newH = s.innerHeight - m.above - m.below;
